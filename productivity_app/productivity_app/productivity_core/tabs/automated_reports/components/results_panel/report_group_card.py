...
class ReportGroupCard(QFrame):
    ...
    def _build_ui(self):
        ...
        # Reports section
        reports_label = QLabel(
            f"Contains {len(self.reports)} report{'s' if len(self.reports) != 1 else ''}")
        ...
        for report_name in self.reports:
            report_badge = self._create_badge(
                report_name, "highlight", max_width=200)
        ...
        if self.tags:
            ...
            for tag in self.tags:
                tag_badge = self._create_badge(
                    tag, "secondaryHighlight", small=True)
    ...
    def _create_badge(
        ...
        if max_width:
            ...
            elided = metrics.elidedText(
                text, Qt.TextElideMode.ElideRight, max_width - 16)
