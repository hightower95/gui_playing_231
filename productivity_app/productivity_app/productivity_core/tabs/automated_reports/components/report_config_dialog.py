...
class ReportConfigDialog(QDialog):
    ...
    def _create_document_input_tab(self, input_name: str) -> QWidget:
        ...
        upload_btn.setIcon(upload_btn.style().standardIcon(
            upload_btn.style().StandardPixmap.SP_ArrowUp))
        server_btn.setIcon(server_btn.style().standardIcon(
            server_btn.style().StandardPixmap.SP_DriveNetIcon))
        cache_btn.setIcon(cache_btn.style().standardIcon(
            cache_btn.style().StandardPixmap.SP_DirIcon))
        ...
        upload_title.setStyleSheet(
            "font-size: 13px; font-weight: bold; color: #e3e3e3;")
        ...
        """)
        choose_files_btn.clicked.connect(
            lambda: self._browse_for_file_enhanced(input_name, selected_file_label))
        upload_dialog_layout.addWidget(
            choose_files_btn, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        setattr(
            self, f"_selected_file_label_{input_name.replace(' ', '_')}", selected_file_label)
