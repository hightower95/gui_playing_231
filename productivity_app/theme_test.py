...
class ThemeTestWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Description
        desc = StandardLabel("Use the buttons below to test different themes", style=TextStyle.NOTES)
        layout.addWidget(desc)
        
        # Theme buttons
        light_btn = StandardButton("Apply Light Theme", role=ButtonRole.PRIMARY)
        light_btn.clicked.connect(self.apply_light_theme)
        layout.addWidget(light_btn)
        
        dark_btn = StandardButton("Apply Dark Theme", role=ButtonRole.SECONDARY)
        ...
        # Sample components
        layout.addWidget(StandardLabel("Sample Components:", style=TextStyle.SECTION))
        
        sample_btn = StandardButton("Sample Button", role=ButtonRole.SUCCESS)
        layout.addWidget(sample_btn)
        
        status = StandardLabel("Status: Theme testing active", style=TextStyle.STATUS)
        layout.addWidget(status)
        
        notes = StandardLabel("This text should be consistent across themes", style=TextStyle.NOTES)
