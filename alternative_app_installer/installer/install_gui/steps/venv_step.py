...
class VenvCreationWorker(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            logging.debug(
                f"VEnv worker: Python executable: {self.python_executable}")
            logging.debug(f"VEnv worker: Target venv path: {self.venv_path}")
            logging.debug(
                f"VEnv worker: Installation path: {self.installation_path}")
            ...
            if self.venv_path.exists():
                logging.info(
                    f"VEnv worker: Found existing venv directory at {self.venv_path}")
                self.progress_queue.put(
                ...
                if self._test_existing_venv():
                    logging.info(
                        "VEnv worker: Existing venv is functional, using it")
                    self.progress_queue.put(
                    ...
                    logging.warning(
                        "VEnv worker: Existing venv is not functional, will recreate")
                    self.progress_queue.put(
                ...
                logging.info(
                    f"VEnv worker: No existing venv found at {self.venv_path}")
            ...
            if not self._create_venv():
                logging.error(
                    "VEnv worker: Virtual environment creation failed")
                self.progress_queue.put(
    ...
    def _test_existing_venv(self):
        ...
        try:
            ...
            logging.debug(
                f"VEnv worker: Testing venv python at: {venv_python}")

            if not venv_python.exists():
                logging.warning(
                    f"VEnv worker: Python executable not found at {venv_python}")
                self.progress_queue.put(
                    "❌ Virtual environment python executable not found")
                return False

            logging.info(
                "VEnv worker: Running 'pip show pip' to test venv functionality")
            self.progress_queue.put(
            ...
            logging.debug(
                f"VEnv worker: pip show pip return code: {result.returncode}")
            if result.stdout:
                logging.debug(
                    f"VEnv worker: pip show pip stdout:\n{result.stdout}")
            if result.stderr:
                logging.debug(
                    f"VEnv worker: pip show pip stderr:\n{result.stderr}")
    ...
    def _create_venv(self):
        ...
        try:
            ...
            logging.info(
                f"VEnv worker: Creating venv with command: {' '.join(cmd)}")
            logging.debug(
                f"VEnv worker: Working directory: {self.installation_path}")
            ...
            logging.debug(
                f"VEnv worker: venv creation return code: {process.returncode}")
            if stdout:
                logging.debug(f"VEnv worker: venv creation output:\n{stdout}")

            if process.returncode == 0:
                logging.info(
                    "VEnv worker: Virtual environment creation completed successfully")
                self.progress_queue.put(
                    "✅ Virtual environment creation completed")
                return True
            else:
                logging.error(
                    f"VEnv worker: Virtual environment creation failed with code: {process.returncode}")
                self.progress_queue.put(
            ...
            logging.error(
                "VEnv worker: Virtual environment creation timed out after 180 seconds")
            self.progress_queue.put("❌ Virtual environment creation timed out")
            return False
        except Exception as e:
            logging.error(
                f"VEnv worker: Error creating virtual environment: {e}")
            self.progress_queue.put(
...
class CreateVenvStep(BaseStep):
    ...
    def __init__(self, installation_settings, shared_state):
        ...
        logging.debug(
            f"VEnv step: Initialized with venv directory name: {self._venv_name}")
    ...
    def complete_step(self) -> bool:
        ...
        if self._is_simulation_enabled():
            logging.info(
                "VEnv step: Simulation mode enabled, completing simulation")
        ...
        try:
            ...
            logging.info(
                f"VEnv step: Setting venv_path in shared state: {venv_path}")
    ...
    def _start_venv_creation(self):
        ...
        if self._creation_in_progress:
            logging.debug(
                "VEnv step: Creation already in progress, ignoring request")
            return
        ...
        logging.info(
            "VEnv step: Starting virtual environment creation/verification process")
        ...
        try:
            ...
            logging.info(
                f"VEnv step: Virtual environment target path: {self._venv_path}")
            logging.info(
                f"VEnv step: Python executable: {self._python_executable}")
    ...
    def _handle_creation_success(self, message: str):
        ...
        logging.info(
            f"VEnv step: Virtual environment creation successful - {message}")
    ...
    def _handle_creation_error(self, error_message: str):
        ...
        logging.error(
            f"VEnv step: Virtual environment creation failed - {error_message}")
