...
def load_launch_config(config_file):
    ...
    defaults = {
        ...
        'venv_python_path': 'c:\Users\peter\OneDrive\Documents\Coding\gui\alternative_app_installer\.test_venv\Scripts\python.exe',
        'venv_dir_path': 'c:\Users\peter\OneDrive\Documents\Coding\gui\alternative_app_installer\.test_venv',
    }
...
class UpdateManagerGUI:
    ...
    def setup_ui(self):
        ...
        current_frame = ttk.LabelFrame(self.root, text="Current Version", padding=10)
        ...
        # Available versions
        versions_frame = ttk.LabelFrame(self.root, text="Available Versions", padding=10)
        ...
        self.version_listbox = tk.Listbox(listbox_frame, font=("Courier New", 9))
        scrollbar = ttk.Scrollbar(listbox_frame, orient="vertical", command=self.version_listbox.yview)
        ...
        ttk.Button(button_frame, text="Refresh", command=self.load_versions).pack(side="left", padx=5)
        ttk.Button(button_frame, text="Install Selected", command=self.install_selected).pack(side="left", padx=5)
    ...
    def load_versions(self):
        ...
        try:
            ...
            versions = get_all_versions(self.venv_python, self.config['library_name'])
    ...
    def install_selected(self):
        ...
        if version == self.current_version:
            messagebox.showinfo("Already Installed", f"Version {version} is already installed.")
            return
        ...
        try:
            # Perform upgrade
            success = upgrade_to_version(self.venv_python, self.config['library_name'], version)
            
            if success:
                messagebox.showinfo("Success", f"Successfully installed version {version}")
                self.current_version = version
                self.load_versions()  # Refresh list
            else:
                messagebox.showerror("Failed", f"Failed to install version {version}")
...
def main():
    try:
        ...
        # Get current version
        current_version = get_installed_version(venv_python, config['library_name'])
        ...
        messagebox.showerror("Startup Error", f"Failed to initialize update manager: {e}")
