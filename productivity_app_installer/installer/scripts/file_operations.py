    ...
    def _copy_utils_folder(self, target_folder: Path, overwrite: bool) -> bool:
        ...
        try:
            ...
            if target_utils.exists() and not overwrite:
                logging.info(f"Utils folder already exists at {target_utils}, skipping")
                return True
    ...
    def _configure_launch_config(self, target_folder: Path, config: Dict[str, Any], overwrite: bool) -> bool:
        ...
        try:
            ...
            if target_config.exists() and not overwrite:
                logging.info(f"launch_config.ini already exists, skipping configuration")
                return True
            ...
            logging.info(f"Created configured launch_config.ini at {target_config}")
            return True
...
"""
    ...
    def validate_installer_structure(self) -> bool:
        ...
        if missing_paths:
            logging.error(f"Missing required installer components: {missing_paths}")
            return False
