...
class Pipeline:
    ...
    def run_provider(self, provider_name: str, **kwargs) -> Any:
        ...
        for precondition in metadata.preconditions:
            if not precondition(self.context):
                raise RuntimeError(
                    f"Precondition failed for provider '{provider_name}'")
        ...
        try:
            ...
            self.context.log(
                f"Provider '{provider_name}' failed: {str(e)}", level="ERROR")
            raise
    ...
    def run_reporter(self, reporter_name: str, **kwargs) -> Any:
        ...
        if missing_inputs:
            raise ValueError(
                f"Missing required input formats: {missing_inputs}")
        ...
        try:
            ...
            self.context.log(
                f"Reporter '{reporter_name}' completed successfully")
            return result

        except Exception as e:
            self.context.log(
                f"Reporter '{reporter_name}' failed: {str(e)}", level="ERROR")
            raise
