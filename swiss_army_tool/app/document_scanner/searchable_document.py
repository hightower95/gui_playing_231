...
class SearchableDocument:
        ...
        try:
            ...
            if self.file_path.suffix.lower() in ['.xlsx', '.xls']:
                ...
                try:
                    ...
                    self.df = pd.read_csv(self.file_path, sep='\t', header=self.header_row)
            
            print(f"  ✓ Loaded '{self.file_name}': {len(self.df)} rows, {len(self.df.columns)} columns")
    ...
    def check_precondition(self, search_term: str) -> bool:
        ...
        try:
            result = eval(self.precondition, {"__builtins__": {}}, {"search_term": search_term})
            return bool(result)
        except Exception as e:
            print(f"  ⚠️  Error evaluating precondition '{self.precondition}': {e}")
            return False
    ...
    def search(self, search_term: str) -> List[SearchResult]:
        ...
        for search_col in self.search_columns:
            ...
            matches = self.df[self.df[search_col].astype(str).str.contains(search_term, case=False, na=False)]
