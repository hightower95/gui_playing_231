...
class SearchView(BaseTabView):
    ...
    def _setup_ui_content(self):
        ...
        self.reload_btn.setToolTip("Reload all documents from disk to pick up any changes")
        search_row.addWidget(self.reload_btn)
        
        header_layout.addLayout(search_row)
        
        # Status label
        self.status_label = QLabel("Ready - Configure documents in Configuration tab")
        ...
        self.results_tree.selectionModel().selectionChanged.connect(self._on_selection_changed)
        ...
        self.context_box.setPlaceholderText("Select a result to view full details...")
    ...
    def display_results(self, results: List[SearchResult]):
        ...
        for doc_results in grouped.values():
            if doc_results:
                num_cols = len(doc_results[0].matched_row_data.keys()) + 1  # +1 for row number
        ...
        for doc_name, doc_results in grouped.items():
            ...
            doc_header_items = []
            doc_item = QStandardItem(f"📄 {doc_name} ({len(doc_results)} result{'s' if len(doc_results) != 1 else ''})")
    ...
    def update_document_count(self, count: int):
        ...
        if count == 0:
            self.update_status("No documents configured - Go to Configuration tab", "orange")
            self.search_btn.setEnabled(False)
            self.reload_btn.setEnabled(False)
        else:
            self.update_status(f"Ready - {count} document(s) configured", "green")
