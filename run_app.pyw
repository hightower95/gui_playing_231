...
def main():
    ...
    if launch_config_file.exists():
        ...
        debug_str = launch_config_parser.get(
            'Settings', 'debug', fallback='false')
        debug_mode = debug_str.lower() in ('true', '1', 'yes', 'on')
        enable_log_str = launch_config_parser.get(
            'Settings', 'enable_log', fallback='false')
        ...
        if enable_log:
            ...
            try:
                ...
                with open(log_file, 'w', encoding='utf-8') as f:
                    ...
                    f.write("Virtual Environment Path: " +
                            str(venv_python) + "\\n")
                    f.write("Venv Exists: " + str(venv_python.exists()) + "\\n")
                    if venv_python.exists():
                        f.write("Venv Directory: " +
                    ...
                    f.write("Launch Config File: " +
                            str(launch_config_file) + "\\n")
                    f.write("Launch Config Exists: " +
                            str(launch_config_file.exists()) + "\\n")
                    ...
                    if launch_config_file.exists():
                        ...
                        try:
                            ...
                            f.write("Error reading config file: " +
                                    str(e) + "\\n")
                    ...
                    f.write("Command: " + str(venv_python) +
                            " -c <runner_script>\\n")
                    f.write("Working Directory: " + str(app_dir) + "\\n")
                    f.write("Creation Flags: " +
                            str(creation_flags if not debug_mode else 0) + "\\n")
                    f.write("\\n")
                    ...
                    if debug_mode:
                        f.write(
                            "Note: Output was also displayed in console (debug mode)\\n\\n")
                ...
                print(
                    "Warning: Could not write comprehensive log file: " + str(log_error))

        # In debug mode, keep console open on any result
        if debug_mode:
            if result.returncode != 0:
                print("\\n*** Application failed with exit code:",
                      result.returncode, "***")
