...
'''
        ...
        if debug_mode:
            # In debug mode: show output in console, don't hide window
            ...
            result = subprocess.run(
                ...
                capture_output=False,  # Let output show directly in console
                creationflags=0  # Don't hide console window
            )
            print("=== Application Finished ===")
            print("Exit Code:", result.returncode)
        else:
            # In normal mode: capture output for logging, hide console
            result = subprocess.run(
                [str(venv_python), "-c", runner_script],
                cwd=str(app_dir),  # Set working directory
        ...
        # Log output to file (only if we captured it in normal mode)
        ...
        try:
            ...
            with open(log_file, 'w', encoding='utf-8') as f:
                ...
                if not debug_mode:
                    # Only log captured output in normal mode
                    ...
                    f.write("Output displayed directly in console (debug mode)\\n")
        except Exception as log_error:
            print("Warning: Could not write log file: " + str(log_error))
